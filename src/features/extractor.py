...
def extract_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    features["error_4xx_ratio"] = (
        features["error_4xx_count"] / features["request_count"]
    )
    features["error_5xx_ratio"] = (
        features["error_5xx_count"] / features["request_count"]
    )
    ...
    if "min_timestamp" in features.columns and "max_timestamp" in features.columns:
        ...
        features["requests_per_second"] = (
            features["request_count"] / features["time_span_seconds"]
        )
    else:
