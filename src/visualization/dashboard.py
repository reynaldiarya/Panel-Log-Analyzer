...
def render_metrics(df: pd.DataFrame):
    ...
    with col4:
        error_5xx = (
            df["status"].apply(lambda x: 500 <= int(x) < 600).sum()
            if not df.empty
            else 0
        )
    ...
    with col_table:
        st.dataframe(
            ip_stats.style.format(
                {"request_count": "{:,}", "unique_urls": "{:,}", "error_4xx": "{:,}"}
            ),
...
def render_anomaly_alerts(anomaly_df: pd.DataFrame, df: pd.DataFrame):
    ...
    whitelisted_count = (
        anomaly_df["is_whitelisted"].sum()
        if "is_whitelisted" in anomaly_df.columns
        else 0
    )
    ...
    with col_info1:
        if cdn_count > 0:
            st.info(
                f"ℹ️ {cdn_count} IPs detected as known CDN (excluded from anomalies)"
            )
    with col_info2:
    ...
    cols[1].metric(
        "High Risk", threat_counts.get("High Risk", 0), delta_color="inverse"
    )
    cols[2].metric(
        "Medium Risk", threat_counts.get("Medium Risk", 0), delta_color="inverse"
    )
    ...
    if not medium_risk.empty:
        ...
        with st.expander(f"View {len(medium_risk)} Medium Risk IPs", expanded=False):
            st.dataframe(
                medium_risk[available_cols].sort_values(
        ...
        ip_reputation_data.append(
            {
                ...
                "Indicators": ", ".join(reputation["indicators"][:3])
                if reputation["indicators"]
                else "-",
            }
        )
...
def render_cdn_summary(anomaly_df: pd.DataFrame):
    ...
    cdn_grouped = (
        cdn_df.groupby("CDN Provider")
        .agg({"IP": "count", "Request Count": "sum"})
        .reset_index()
    )
...
def render_attack_indicators(df: pd.DataFrame):
    ...
    with col2:
        ...
        post_suspects = (
            df[df["method"].str.upper() == "POST"]
            .groupby("ip")
            .agg(
                {"url": "count", "status": lambda x: (x.astype(int) // 100 == 4).sum()}
            )
